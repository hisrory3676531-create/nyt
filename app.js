(() => {
  ...
  const shoppingListContent = el("shoppingListContent");
  ...
  const getRecentHistory = (daysBack = 5) => {
    const history = { dishes: new Set(), ingredients: new Set(), fishCount: 0, meatCount: 0 };
    if (!appState.run || !appState.run.planByDate) return history;

    const dates = Object.keys(appState.run.planByDate).sort().reverse();
  ...
  const pickMeal = (category, exclusions, dayIndex) => {
    ...
    // Фильтруем по исключениям и правилу 5 дней
  ...
  const buildPlanForDay = (nutrition, exclusions, dayIndex = 0) => {
    const splits = {
      breakfast: 0.25, // Энергичное, но легкое начало
      lunch: 0.35,     // Самый плотный прием пищи
      dinner: 0.25,    // Умеренный сытный вечер
      snack: 0.15,     // Без изменений
    };
    ...
    return {
      breakfast: makeMeal("breakfast"),
      lunch: makeMeal("lunch"),
      dinner: makeMeal("dinner"),
      snack: makeMeal("snack"),
    };
  };
  ...
  const computeDaySuccess = (isoDate) => {
    ...
    const allMeals = Object.values(day.eaten).every(Boolean);
  ...
  const showToast = (message, type = "success") => {
    ...
    toast.className = `fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 rounded-2xl text-sm font-bold shadow-2xl transition-all duration-300 transform translate-y-20 opacity-0 flex items-center gap-3 ${
  ...
  const smartReplaceMeal = async (date, category, cardEl) => {
    ...
    let possible = list.filter(m => {
      ...
      return !isExcluded && !isRepeated && !isCurrent;
    });

    if (possible.length > 0) {
      const nutrition = appState.run.nutrition;
      // Используем те же веса, что и при основной генерации
      const splits = {
        breakfast: 0.25,
        lunch: 0.35,
        dinner: 0.25,
  ...
  const replaceMealWithAI = async (date, category, cardEl) => {
    if (!GEMINI_API_KEY || GEMINI_API_KEY.includes("AIzaSyCoc86_dpfX8iKVzr-PCoiO-99tyx4-LMA")) {
      const nutrition = appState.run.nutrition;
      const exclusions = appState.profile.exclusionsNormalized;
      const newMeal = buildPlanForDay(nutrition, exclusions)[category];
      appState.run.planByDate[date][category] = newMeal;
      saveStorage();
      renderDashboard();
      return;
    ...
    try {
      ...
      alert("Не удалось заменить блюдо. Попробуйте позже.");
      renderDashboard();
    }
  };
  ...
  const generateShoppingList = () => {
    ...
    const groups = {
      ...
      "Крупы и Бакалея": ["гречка", "рис", "макароны", "перловка", "овсянка", "манка", "пшено", "ячка", "хлеб", "хлебец", "картофель", "мука", "сахар", "соль", "специи"],
  ...
  const validateProfile = (p) => {
    ...
    if (!p.activity) missing.push("активность");
    if (!p.goal) missing.push("цель");
    if (!Number.isFinite(p.durationDays)) missing.push("длительность");
...
const fetchGeminiPlan = async (profile, nutrition, durationDays) => {
    ...
    const prompt = `
      Ты профессиональный нутрициолог. Создай план питания на ${durationDays} дней для марафона.
      ПРОДУКТЫ ДОЛЖНЫ БЫТЬ ПРОСТЫМИ, ДОСТУПНЫМИ И НЕДОРОГИМИ (для обычного супермаркета).
      Данные пользователя: Возраст ${profile.age}, ${profile.gender}, вес ${profile.weightKg}кг, рост ${profile.heightCm}см.
      Цель в день: ${nutrition.calories} ккал, Б: ${nutrition.proteinG}г, У: ${nutrition.carbsG}г, Ж: ${nutrition.fatG}г.
      Исключить из рациона: ${profile.exclusionsDisplay.join(", ")}.
      Ответ СТРОГО в формате JSON, где ключи — даты YYYY-MM-DD, начиная с ${appState.run.startDate}.
      Каждый день содержит: "breakfast", "lunch", "dinner", "snack". 
      В каждом приеме пищи: 
      "name" (строка), 
      "ingredientsWithWeights" (массив строк с указанием граммовки, например: "Куриное филе — 150г"), 
      "macros" (объект: calories, proteinG, carbsG, fatG),
      "instructions" (массив строк с шагами приготовления).
      Используй только русский язык. Только чистый JSON без markdown.
    `;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Gemini API Error Full Details:", errorData);
        throw new Error(`Ошибка API (${response.status}): ${errorData.error?.message || "Модель не найдена"}`);
      }

      const data = await response.json();
      let rawText = data.candidates[0].content.parts[0].text;
      
      // Очистка от markdown на случай, если ИИ его добавил
      const cleanJson = rawText.replace(/```json|```/g, "").trim();
      return JSON.parse(cleanJson);

    } catch (err) {
      console.error("Gemini Error:", err);
      throw new Error(`Сбой генерации: ${err.message}`);
    }
};
  ...
  const startMarathon = async () => {
    ...
    const exclusionsDisplay = parseExclusionsDisplay(exclusionsInput.value || "");
    const exclusionsNormalized = normalizeExclusions(exclusionsDisplay);
    const durationDays = selectedDays;
    const goal = selectedGoal;

    const missing = validateProfile({ age, gender, heightCm, weightKg, activity, durationDays, goal });
    if (missing.length) {
      onboardingError.textContent = `Пожалуйста, заполните: ${missing.join(", ")}.`;
    ...
    appState.profile = {
      ...
      activity,
      exclusionsDisplay,
      exclusionsNormalized,
      goal,
      durationDays
    };

    const nutrition = computeNutrition(appState.profile);
    ...
    try {
      const plan = await fetchGeminiPlan(appState.profile, nutrition, durationDays);
  ...
  const setDurationUI = (days) => {
    ...
    duration7Btn.classList.toggle("bg-emerald-50", is7);
    duration7Btn.classList.toggle("text-emerald-800", is7);
    duration7Btn.classList.toggle("ring-emerald-200", is7);
    duration7Btn.classList.toggle("bg-white", !is7);
    duration7Btn.classList.toggle("text-slate-700", !is7);
    duration7Btn.classList.toggle("ring-slate-200", !is7);

    duration14Btn.classList.toggle("bg-emerald-50", !is7);
    duration14Btn.classList.toggle("text-emerald-800", !is7);
    duration14Btn.classList.toggle("ring-emerald-200", !is7);
    duration14Btn.classList.toggle("bg-white", is7);
    duration14Btn.classList.toggle("text-slate-700", is7);
    duration14Btn.classList.toggle("ring-slate-200", is7);
  };
  ...
  const setGoalUI = (goal) => {
    ...
    document.querySelectorAll(".goal-btn").forEach((btn) => {
      ...
      btn.classList.toggle("ring-emerald-200", isActive);
